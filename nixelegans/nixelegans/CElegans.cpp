...
CElegans::CElegans(const double i_length, const int i_size)
{
	size = i_size;
	time = 0;
	length = i_length;
...
//	FILE *fout = fopen("celegans280positions.new.txt","wt");
	...
	if(f_pin)
	{
		...
		while(!f_pin.eof())
		{
            ...
            Vector3D temp_v =vshift+Vector3D( dx * ( -1.5 - x*size) , y, -dl/2+(0.045 + z/25) / 0.095 * length);
            addNeuron(name, temp_v, (float)tresh, type,clr_index);
			i++;
		...
		while(!fconn_in.eof())
		{
			...
			if((ctype=="EJ")||(ctype[0]=='S'))
			{
				for(i=0;i<(int)neuron.size();i++)
					if((neuron[i] != NULL) && neuron[i]->getName()==name)
					{
						status=1;
						break;
					}
...
void CElegans::step(std::string neuron_name, float signal_strength)
{
	unsigned int i;
...
//Spring friction added;
    ...
    mode3 = false; // set true and uncomment mode3 block to send singals directly to muscles, rather than through neurons first.
    mode4 = true; //false;
/*
	if(mode3)
	{
        ...
        //printf("all muscles printed... exiting.\n");
        //exit(0);
...
*/
	...
	if(mode4)
	{
		...
		for(i=0;i<neuron.size();i++)
		{
            ...
            if (! n_name.compare(neuron_name)){
                
                printf("neuron '%s' firing with strength: %.5f\n",neuron_name.c_str(), signal_strength);
                neuro_signal = signal_strength;
                neuron[i]->getSignal(neuro_signal);
                
            }
            else {
                neuron[i]->getSignal(0);
            }
            // original code that generated the signals.
            /*
			if((n_name[0] == 'V' && n_name[1]=='B')||(n_name[0] == 'D' && n_name[1]=='B'))
			{
                
                printf("firing %s, ",n_name.c_str());
        ...
        //printf("All neurons printed (%zu)... exiting.\n", neuron.size());
        //exit(0);
	}

    float dt = .009f;// NOTE: this is what main.cpp -> MaxPossible_dt is set to... if that changes, this should too, unless we figured out that it shouldn't.
	time += dt;//0.002f;
	/**/
	for(i=0; i<mPoint.size();++i)
	{
...
/**
 original (well, original after being ported.)
 */
...
//Spring friction added;
	...
	/**/
    mode3 = false;//true;
    // Making sure mode4 is set to false
    mode4 = true; //false;
	if(mode3)
	{
		float neuro_signal;
