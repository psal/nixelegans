...
extern float scale;
// Sean more stuff missing
const float neuron_radius = 0.005f;


// SEAN --> Where the printing happens, except this is unused.
extern GLvoid glPrint(GLint x, GLint y, const char *string, ...);
// SEAN --> Where something undocumented should happen, but again, unused.
...
void CSynapse::checkActivity()
{
	...
	*/
	...
	// SEAN --> changed "min" to "fmin"
	income = fmin(income,1.f);
...
/**/
	if(selected)
	{
		//if(name.length()<5)
		// SEAN --> changed "min" to "fmin"
		// SEAN --> changed "byte" to "GLubyte"
		glPrint2(rpos.x,rpos.y,rpos.z,(GLubyte)fmin(255,r*1.7),(GLubyte)fmin(255,g*1.7),(GLubyte)fmin(255,b*1.7),name.c_str());
	}
...
/**/
	...
	*/
	...
	/*	if(axon[j]->getTargName().length()<5)
		...
		//glPrint2(rpos2.x,rpos2.y,rpos2.z,150,150,150,axon[j] ->getTargName().c_str());
		// SEAN --> changed "min" to "fmin"
		glColor3ub((GLubyte)fmin(255,r*0.7),(GLubyte)fmin(255,g*0.7),(GLubyte)fmin(255,b*0.7));
		...
		{
			// SEAN --> changed "min" to "fmin"
			glColor3ub((GLubyte)fmin(255,r*0.75),(GLubyte)fmin(255,g*0.75),(GLubyte)fmin(255,b*0.75));
		}
