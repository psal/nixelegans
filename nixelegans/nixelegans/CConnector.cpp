...
void CSpring::draw()
{
    ...
    /** // sean said to change the below two lines to the two lines below them
     ...
     */
    Vector3D rpos  = (ort1*(p1->getX()-pos_rc.x) + ort2*(p1->getY()-pos_rc.y) + ort3*(GroundHeight-0.1-pos_rc.z))*scale*0.0001 + vcenter;
    Vector3D rpos2 = (ort1*(p2->getX()-pos_rc.x) + ort2*(p2->getY()-pos_rc.y) + ort3*(GroundHeight-0.1-pos_rc.z))*scale*0.0001 + vcenter;
